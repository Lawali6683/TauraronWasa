// /pages/functions/api/champions.js

export async function handleChampions(request) {
  return championsWorker(request);
}

// --- Babban Champions League Worker ---
async function championsWorker(request) {
  ...
  // 🔐 Duba POST + JSON kawai ake yarda
  ...
  try {
    ...
    const CL_MATCHES_URL = `https://api.football-data.org/v4/competitions/CL/matches${
      stage ? `?stage=${stage}` : ""
    }`;
    const DED_TABLE_URL =
      "https://api.football-data.org/v4/competitions/DED/standings";
    const CL_SCORERS_URL =
      "https://api.football-data.org/v4/competitions/CL/scorers?limit=10";

    // A dauko duk bayanan lokaci guda
    ...
    return new Response(
      JSON.stringify({
        error: true,
        message: "Server error while fetching Champions data.",
        details: e.message
      }),
      {
        status: 500,
