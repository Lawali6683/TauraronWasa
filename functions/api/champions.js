...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!matchesResponse.ok) {
      ...
      console.error(`Error fetching matches: ${matchesResponse.status} - ${errorText}`);
      const response = new Response(
        JSON.stringify({ error: true, message: `Failed to fetch matches: ${matchesResponse.statusText}` }),
    ...
    const errorResponse = new Response(
      JSON.stringify({
        error: true,
        message: "Server error while fetching Champions League data.",
        details: e.message,
      }),
      {
        status: 500,
