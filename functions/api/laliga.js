export async function onRequest(context) {
  ...
  try {
    ...
    // Sabunta URLs don La Liga
    const matchesUrl = `${BASE_API_URL}/competitions/${PD_CODE}/matches`;
    const standingsUrl = `${BASE_API_URL}/competitions/${PD_CODE}/standings`;
    const scorersUrl = `${BASE_API_URL}/competitions/${PD_CODE}/scorers?limit=10`;
    ...
    const finalData = {
      ...
      // An tabbatar da tsarin standings ya yi daidai da na La Liga
    ...
    const errorResponse = new Response(
      JSON.stringify({
        error: true,
        message: `Kuskure a wajen dauko bayanan La Liga: ${e.message}`,
      }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    }
