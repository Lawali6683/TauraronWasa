export async function onRequest(context) {
  ...
  try {
    ...
    // Sabunta URLs don Ligue 1
    const matchesUrl = `${BASE_API_URL}/competitions/${FL1_CODE}/matches`;
    const standingsUrl = `${BASE_API_URL}/competitions/${FL1_CODE}/standings`;
    const scorersUrl = `${BASE_API_URL}/competitions/${FL1_CODE}/scorers?limit=10`;
    ...
    const finalData = {
      ...
      // An tabbatar da tsarin standings ya yi daidai da na Ligue 1
    ...
    console.error("Server error in ligue1.js:", e.message, e.stack);
    const errorResponse = new Response(
      JSON.stringify({
        error: true,
        message: `Kuskure a wajen dauko bayanan Ligue 1: ${e.message}`,
      }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    }
